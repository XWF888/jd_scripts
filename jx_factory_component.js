 ...
 * @LastEditors: whyour
 * @LastEditTime: 2021-03-23 22:23:15
 ...
 * 点击厂长任务
  quanx:
  [task_local]
  0 1,18 * * * jx_factory_component.js, tag=京喜工厂plus, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdgc.png, enabled=true
...
$.showLog = $.getdata('gc_plusShowLog') ? $.getdata('gc_plusShowLog') === 'true' : true;
...
function pickUserComponents(pin, isMe) {
  return new Promise(async resolve => {
    $.get(taskUrl('usermaterial/GetUserComponent', `pin=${pin}`, '_time,pin,zone'), async (err, resp, data) => {
      try {
...
function pickUpComponent(placeId, pin, isMe) {
  return new Promise(async resolve => {
    $.get(taskUrl('usermaterial/PickUpComponent', `pin=${pin}&placeId=${placeId}`, '_time,pin,placeId,zone'), (err, resp, data) => {
      try {
...
function taskUrl(function_path, body, stk) {
  let url = `${JD_API_HOST}dreamfactory/${function_path}?zone=dream_factory&sceneval=2&g_login_type=1&&_time=${Date.now()}&_=${Date.now()}&${body}`
